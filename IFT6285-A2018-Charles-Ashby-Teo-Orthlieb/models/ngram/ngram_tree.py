...
class NG_Tree(Model):
	...
	def _getsize(node):
		...
		for word, child in node.items():
			size += getsizeof(child)+getsizeof(child[0])+NG_Tree._getsize(child[0])
		return size

	def getsize(self):
		size = 0
		for word, child in self.root.items():
			size += getsizeof(word)+getsizeof(child)+getsizeof(child[1])+getsizeof(child[0])+NG_Tree._getsize(child[0])
		return size
